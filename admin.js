// HDV Admin v2.8 - Updated 2026-02-22
...
function mostrarProductosGestion() {
    ...
    productosFiltrados.forEach(prod => {
        ...
        const imgHTML = prod.imagen 
            ? `<img src="${prod.imagen}" class="img-thumb" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" title="Click para cambiar">
               <div class="img-thumb-placeholder" style="display:none;" onclick="editarImagenProducto('${prod.id}')" title="Agregar imagen">📷</div>`
            : `<div class="img-thumb-placeholder" onclick="editarImagenProducto('${prod.id}')" title="Agregar imagen">📷</div>`;
        ...
        tr.innerHTML = `
            ...
            <td style="cursor:pointer;" onclick="editarImagenProducto('${prod.id}')">${imgHTML}</td>
...
function previsualizarNuevaImagen(url) {
    const container = document.getElementById('previewNuevaImagen');
    if (url && url.match(/^https?:\/\/.+/)) {
        container.innerHTML = `<img src="${url}" style="max-width:120px;max-height:80px;border-radius:8px;border:2px solid #e5e7eb;" onerror="this.parentElement.innerHTML='<span style=\\'color:#ef4444;font-size:13px;\\'>⚠️ URL inválida</span>'">`;
    } else {
        container.innerHTML = '';
    }
}
...
function agregarNuevoProducto() {
    ...
    document.getElementById('previewNuevaImagen').innerHTML = '';
...
function actualizarCliente(id, campo, valor) {
    ...
    if (c) { c[campo] = valor; registrarCambio(); }
}
    ...
    reader.onload = (e) => {
        try {
            ...
            if (agregados > 0) {
                ...
                mostrarMensajeHerramientas(`${agregados} productos importados. Usa "Guardar y Descargar" para aplicar.`);
            }
        } catch (error) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (agregados > 0) {
                ...
                mostrarClientesGestion();
                for (let i = 0; i < agregados; i++) registrarCambio();
                mostrarMensajeHerramientas(`${agregados} clientes importados. Usa "Guardar y Descargar" para aplicar.`);
            }
        } catch (error) {
            alert('Error al importar: ' + error.message);
        }
